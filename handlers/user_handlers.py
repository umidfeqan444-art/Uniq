...
from config import SHOP_NAME, ADMIN_ID, CRYPTO_PAY_API_KEY, MIN_TOPUP_AMOUNT
...
async def process_top_up_amount(message: Message, state: FSMContext):
    try:
        ...
        await state.update_data(top_up_amount=amount, invoice_id=invoice['invoice_id'])
...
💡 After payment, click "🔄 Check payment" to verify status."""
        ...
        msg = await safe_send_photo(message, "❌ Please enter a valid amount (number). Example: 0.05 or 1.5",
            parse_mode="HTML"
        )
        await _save_last_message(message.from_user.id, msg, True)
...
async def top_up_handler(callback: CallbackQuery, state: FSMContext):
    text = f"""💳 Write top-up amount (minimum: ${MIN_TOPUP_AMOUNT:.2f})"""
